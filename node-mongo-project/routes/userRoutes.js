...
router.post('/track', async (req, res) => {
  try {
    const { type, detail, token } = req.body; // <--- Get the v3 token
    ...
    if (!googleData.success || googleData.score < 0.5) {
        console.log(`connection disrepency, verification: ${googleData.score}`);
        return res.status(403).json({ message: "Bot activity detected." });
    }

    // If Human, saves token
    const newEvent = new Analytics({ type: type, detail: detail });
    await newEvent.save();
    
    res.status(200).send('Tracked safely');
